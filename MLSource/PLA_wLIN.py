...
def createTestDataSet():#数据样本
    testData = [   [1, 1, 1],
                   ...
                   [1, 1, 3]]
    return testData
def linear_regression(trainDataIn,trainlabelIn):
    w_LIN=dot(dot(np.linalg.pinv(dot(trainDataIn.T,trainDataIn)),trainDataIn.T),trainlabelIn)
    return w_LIN
def pla(traindataIn,trainlabelIn):#PLA计算
    ...
    m,n=shape(traindata)
    #w=ones((n,1))
    w=linear_regression(traindata,trainlabel).getA()
    #w=(mat([1,3,1]).T).getA()
