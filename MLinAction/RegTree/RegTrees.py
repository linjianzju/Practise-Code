...
#dataMat1=loadDataSet('ex0.txt')
#retTree1=createTree(mat(dataMat1))
    
#dataMat=loadDataSet('ex2.txt')
#retTree=createTree(mat(dataMat),ops=(0,1))  #ops是预剪枝，切分过程中发现切分误差<ops[0]或切分数据集<ops[1]都不切分
#dataMatTest = loadDataSet('ex2test.txt')
#treeTest=prune(retTree, mat(dataMatTest))  #prune是后剪枝，全部切分完后，对测试数据集进行切分，从叶子节点出发，如果不切分误差小于切分误差则合并切分
    
#dataMat=loadDataSet('exp2.txt')
#retTree=createTree(mat(dataMat),leafType=modelLeaf,errType=modelErr,ops=(1,10))
